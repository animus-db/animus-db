...
- **Status:** PR 1 (docs) and PR 2 (Transact groundwork) landed
  ...
  grace-check, unrelated to and unmodified by the relay fix — still blocks
  the scenario from converging**, so
  `coordinator_never_finished_past_prepare_recovers_atomically` and its
  `_over_seeds` sibling remain `#[ignore]`d, with the full two-stage
  diagnosis in their own doc comment. See ADR 0061's "#731 closed"
  addendum and `docs/engineering-lessons.md`'s matching entries. Issue
  #731 itself is closed; a new issue is to be filed against
  `txn_recover`. PRs 4-7 (the wire corpus, PartiQL siblings, PartiQL sim
  tests, docs close-out) remain open.
...
| 7 | C-06 (PRs 1-3 landed 2026-09-07, issue #731 fixed 2026-09-07, a second txn_recover finding to be filed; PRs 4-7 open) | Gated on C-04 (closed 2026-09-07) — the D4 `Reconciler` and D3 generic dispatch cores it builds on |
