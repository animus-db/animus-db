...
| [0041](0041-materialized-secondary-indexes.md) | Materialized DynamoDB secondary indexes: a GSI is a hidden table maintained asynchronously by a derivative drain over a durable per-tablet change log (the log DynamoDB Streams will later consume), an LSI is colocated in the base tablet and atomic with the base write, plus a row-kind discriminator in the key layout and the deletion of the edge-local in-memory index (amends 0013, 0022, 0023; depends on 0018, 0024, 0034) | Proposed |
