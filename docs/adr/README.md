...
| [0042](0042-dynamo-streams.md) | DynamoDB Streams: semantics, API, and the change-log lifecycle — per-item ordering + eventual consistency via copier lag, the `DynamoDBStreams_20120810` API surface, view-type projection at read, gapless per-shard sequence numbers, non-expiring stateless iterators, a leader-local `GetRecords` read with no `ReadIndex` barrier, and a genuine multi-consumer change-log cursor (HLC watermark rows + the min-over-rows split/merge rule) generalizing ADR 0041 §4's single-consumer design (amends 0013, 0024, 0028, 0033, 0034, 0035, 0041; depends on 0018, 0043) | Proposed |
| [0043](0043-stream-shard-subsystem.md) | The stream-shard subsystem: DynamoDB stream shards as the fixed, never-split/merged tablets of a hidden per-stream table, placed only on a new dedicated **streams** node role; `StreamAppend`/`StreamTrim` (apply-assigned gapless positions, per-partition-key dedupe, fence-less) over three new row kinds; `CreateStreamShards` atomic-N provisioning; and a unified per-node change-consumer loop (GSI drain + stream copier + trim janitor) replacing the standalone index drain (amends 0013, 0024, 0028, 0033, 0034, 0035; depends on 0018, 0031, 0042) | Proposed |
