...
- **The live endpoint is wired in `animusd`.** A running node serves
  ...
  - **It aggregates the node's three role sinks.** A node runs three internal
    `ProdEnv` roles on distinct ids (control / data / coord), and each records
    into its **own** sink (`RaftNode::start` records into the control env's sink;
    the replica and the coordinator into theirs). The handler
    ...
    data-plane counters surface from one endpoint; today only the control-plane
    counters move, and any data-plane counter surfaces automatically once recorded
    with no endpoint change. Proven over real TCP in
...
- **Deferred:** data-plane and storage-engine counters (read/write quorums,
  ...
  deterministic bucketing scheme).
