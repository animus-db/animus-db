...
| G5 | Issue #298 (exactly-once duplication/deficit at a split boundary, root cause unknown, pre-existing) reproduces far more readily under in-place's higher splits-per-unit-time — must be resolved (or the affected soak deliberately re-tuned) before rung 4's copy-deletion layer, which removes the option to pin the affected test to `Copy` | **Open, blocking rung 4's remaining layer specifically** (not rungs 1–3, not the default-flip layer itself, which pinned around it) — surfaced 2026-08-25 auditing tests for the default flip; see this ADR's own rung-4-layer-2 as-built note and `docs/engineering-lessons.md`'s matching entry for the full finding. |
