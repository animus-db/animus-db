# CustosDB — Bootstrap Brief for Claude Code

You are bootstrapping a new open-source project, **CustosDB**, and delivering its
...
CustosDB is a **masterless, linearly-scalable, open-source NoSQL database written
...
└── crates/
    ├── custos-env            # Env traits + ProdEnv + SimEnv handle
    ├── custos-sim            # the deterministic simulator (clock, net, disk, scheduler, faults)
    ├── custos-storage        # StorageEngine trait + in-memory impl (+ rocksdb feature)
    ├── custos-control        # control-plane RSM: metadata model, Raft log, epochs, reconciler
    ├── custos-data           # leaderless data plane: quorum read/write, routing, fencing
    ├── custos-tablet         # tablet model (later: split/merge/placement)
    ...
    ├── custos-cql            # CQL adapter (later)
    ├── custos-dynamo         # DynamoDB adapter (later)
    ├── custos-test           # Elle-style history recorder + checker; shared test harness
...
- `custos-env`: traits `Clock`, `Rng`, `Network`, `Disk`, `Spawner`, and an `Env`
...
- `custos-sim`: `SimEnv` over a single shared `SimState` —
...
- `custos-storage`: a `StorageEngine` trait driven by what the distributed layer
...
- `custos-control`: a Raft-replicated metadata state machine (via `openraft`) holding
...
- `custos-data`: leaderless quorum write/read for a single tablet; routing via the
...
- `custos-test`: a `Recorder` logging `invoke`/`ok`/`fail`/`info` entries with
