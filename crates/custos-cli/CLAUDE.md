...
The operator/client CLI (`custos`): a thin plain-TCP client over a node's
request/reply API. Subcommands: `status`, `put`, `get`.

## What's non-obvious

- It depends on `custosd` only for the shared protocol types and frame
...
- `<node-addr>` is any node's client address (as printed by `custosd --cluster`).
