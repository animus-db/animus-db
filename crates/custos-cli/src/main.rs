//! CustosDB operator/client CLI (`custos`).
//!
//! Usage:
//!   custos status <node-addr>
//!   custos put    <node-addr> <key> <value>
//!   custos get    <node-addr> <key>
//!
//! A thin client over a node's request/reply API (see `custosd`). `<node-addr>`
//! is a node's client address as printed by `custosd`.

use std::process::ExitCode;

use custosd::{ClientRequest, ClientResponse, read_frame, write_frame};
