...
- `src/check.rs` — `check_cycles` (serializability), `check_strict_cycles`
  (**strict** serializability: the same graph plus real-time precedence edges,
  i.e. linearizability for single-object ops), `check_durability`,
  `check_convergence`; each returns a `CheckReport` carrying the run seed.
  `realtime_edge_count` reports how many real-time edges a history admits, so a
  caller can prove its strict verdict is not vacuous.
...
- `negative_control.rs` — the **teeth proof**, shared by every corpus below
  ...
  corpus run.
- `strict_negative_control.rs` — the same for `check_strict_cycles`, and it
  pins the boundary from **both** sides: the stale-read history plain
  `check_cycles` accepts and the strict check rejects (the two verdicts on one
  history are the whole point), *and* the concurrent-window read the strict
  check must keep accepting, so it cannot decay into "any stale read is a
  bug."
...
- `raftkv_linearizable.rs` — proves `animus-cp-data`'s leaderful data plane
  **linearizable** under contention. **Self-contained** — it reuses only the
  *checkers* (`check_cycles`/`check_strict_cycles`/`check_durability`/
  `check_convergence`) and the
...
- **It takes two checkers, and `check_cycles` alone is not enough** — despite
  this file's name, and despite what this guide claimed until the strict check
  landed. A single Raft group *is* the serialization authority, so a **fork** (an
  observed list that is not a prefix of the recovered order) is caught by
  `recover`. **Staleness is not**: this workload puts one mop per transaction, so
  a read-only transaction's only outgoing edges are `rw` to appenders it missed
  and its only incoming edges are `wr` from appenders it saw, `ww` runs strictly
  forward, and a surviving read saw a prefix — everything it missed sorts after
  everything it saw and no path leads back. A read-only transaction can therefore
  **never lie on a data-dependency cycle**, so a read served from stale state is
  perfectly serializable (order it earlier) while being flatly non-linearizable —
  exactly what `leader_minority`/`split_brain` create. `check_strict_cycles` adds
  the real-time precedence edges that catch it, and each scenario also asserts
  `realtime_edges > 0`, because the strict check only bites where operations do
  not overlap. There is no eventually-consistent read path here to manufacture
  torn-read false positives, so every check runs on this one layer. Convergence +
  durability are still *eventual* (a lagging follower catches up via log/
  snapshot), so they use the same **converged-or-timeout** poll every corpus in
  this crate does (see "What's non-obvious" above).
