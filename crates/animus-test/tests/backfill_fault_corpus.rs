...
/// Depth-scaled per-cell seed derivation and expansion — shared with
/// every sibling corpus in this crate, `animus_test::corpus`
/// (`odd_name_seed`/`seeds_from_env`/`for_each_seed`).
fn for_each_seed(name: &str, body: impl FnMut(u64)) {
    corpus::for_each_seed(name, corpus::seeds_from_env("ANIMUS_BACKFILL_SEEDS"), body);
}
