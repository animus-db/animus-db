...
//! whole run is a pure function of its seed: replay with `CUSTOS_SEED=<seed>`.

use std::collections::BTreeMap;
use std::sync::{Arc, Mutex};
use std::time::Duration;

use custos_control::{MetaCommand, RaftNode};
use custos_data::{DataClient, ReadResult, Router, serve_anti_entropy, serve_replica};
...
use custos_test::history::{Mop, Process};
use custos_test::{Recorder, check_convergence, check_cycles, check_durability};
...
fn assembled_system_stays_consistent_under_faults() {
    let seed = std::env::var("CUSTOS_SEED")
        .ok()
