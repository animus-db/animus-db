...
impl Metric {
    ...
    ///
    /// **`KindEvalSeatbeltMismatch` (ADR 0054 step 3) was removed in step
    /// 4b**, alongside the leader-side prediction it counted
    /// (`animusd::dynamo::predict_kind_eval_decision`/
    /// `report_kind_eval_seatbelt_mismatch`) — apply's decision is the only
    /// one computed at all now, so there is nothing left to compare it
    /// against. Its slot is not reused; every remaining variant's own slot
    /// stays stable.
