...
- `Query` / `Scan` over the **distributed** plane: the data plane
  (`custos-data`) has no quorum range scan, so the registry tracks written item
  ...
  `custosd` quorum-reads each key. The key index (and the schema map) are
...
- **Still deferred** (don't represent as a full adapter): projection
  ...
  filter grammar. `custos-cql` would map onto the same core the same way.
...
`custosd`'s `tests/dynamo_wire.rs` (Put/Get/Delete), `tests/dynamo_extended.rs`
