...
- `Epoch` is the **data-plane fencing token**: every placement change bumps it.
  The actual split/merge *state transitions* live in `custos-control`'s
...
`cargo test -p custos-tablet` — inline unit tests for `contains`, `split_at`
bounds, and `abuts`.
