...
//! `custos_cql` crate, then routed through the **same** quorum coordinator the
...
//!
//! - the **schema catalog** ([`custos_cql::Catalog`]) — *in-memory, not
...
//!   connections.
//!
//! The pure protocol/type/catalog logic stays in `custos_cql`; only the socket
...
async fn prepare(
    ...
    let stmt = match custos_cql::parse_statement(cql) {
        Ok(s) => s,
    ...
    let (bind_specs, keyspace, table) = match &stmt {
        Statement::Insert(ins) => {
            match custos_cql::plan::insert_bind_types(&guard.catalog, selected.as_deref(), ins) {
                Ok(specs) => {
                    let ks = ins
                        .keyspace
                        .clone()
        ...
        Statement::Select(sel) => {
            match custos_cql::plan::select_bind_types(&guard.catalog, selected.as_deref(), sel) {
                Ok(specs) => {
                    let ks = sel
                        .keyspace
                        .clone()
...
async fn run_cql(
    ...
    match stmt {
        ...
        Statement::CreateTable(ct) => {
            ...
            let schema = custos_cql::schema_of(&ct);
            let mut guard = state.lock().await;
            match guard
                .catalog
        ...
        Statement::Insert(ins) => {
            let plan = {
                let guard = state.lock().await;
                custos_cql::plan_insert(&guard.catalog, session.keyspace.as_deref(), &ins, binds)
            };
            match plan {
        ...
        Statement::Select(sel) => {
            let plan = {
                let guard = state.lock().await;
                custos_cql::plan_select(&guard.catalog, session.keyspace.as_deref(), &sel, binds)
            };
            match plan {
...
async fn run_select(ctx: &ClientCtx, stream: i16, plan: ReadPlan) -> Vec<u8> {
    ...
    match read {
        Some(bytes) => {
            ...
            response::typed_rows_result(stream, "custos", &table, &projection, Some(&cells))
        }
        None => response::typed_rows_result(stream, "custos", &table, &projection, None),
    }
}
...
/// keyed by schema index; see `custos_cql::plan`).
fn build_row(
    ...
    let decoded = custos_cql::decode_row(stored).map_err(|e| e.to_string())?;
