...
//!
//! A [`ClusterConfig`] lists every node's four listen addresses (control / data
//! / coord internal `ProdEnv` roles + the client API) plus the quorum sizes. One
//! `custosd` process runs one node by index: it binds *its* listeners at the
//! configured addresses and learns every peer's address from the same config.
//!
