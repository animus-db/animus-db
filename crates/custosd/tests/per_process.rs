...
//! form one cluster and serve clients. This mirrors running one `custosd`
//! process per node, but in-process so the test needs no child processes.

use std::net::SocketAddr;
use std::time::Duration;

use custosd::{ClientRequest, ClientResponse, ClusterConfig, Node, RoleAddrs, read_frame};
...
async fn per_process_nodes_form_a_cluster_from_shared_config() {
    ...
    for i in 0..n {
        let node = custosd::run_node(&config, i, dir.path().join(format!("node-{i}")))
            .await
