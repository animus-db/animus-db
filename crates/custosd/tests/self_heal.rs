//! Observable **self-healing** in the assembled `custosd` node (ADR 0005 + 0012).
//!
//! The autonomous control-plane behaviors are proven deterministically in
//! `custos-control` under `SimEnv`; this test proves they are actually **wired
...
//!    surviving replicas.
//!
//! Like the other `custosd` tests this is real TCP/time, so it polls with
//! generous timeouts rather than asserting deterministic timing.
//!
//! There is also a concurrency smoke test (mirroring
//! `custos-storage/tests/lsm_concurrent.rs`): many clients hammering the
//! assembled node concurrently complete without deadlock.

use std::net::SocketAddr;
use std::time::Duration;

use custosd::config::data_id;
use custosd::{ClientRequest, ClientResponse, Node, bind_cluster, start_cluster};
    ...
    let healed = async {
        loop {
            ...
            let down = meta
                .members
                .get(&victim_data_id)
                .is_some_and(|m| m.status == custosd::NodeStatus::Down);
