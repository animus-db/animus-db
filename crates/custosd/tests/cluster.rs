...
//! through the client request API exactly as the `custos` CLI does.
//!
...
use custosd::{ClientRequest, ClientResponse, Node, bind_cluster, read_frame, start_cluster};
...
async fn call(addr: std::net::SocketAddr, req: ClientRequest) -> ClientResponse {
    ...
    custosd::write_frame(&mut stream, &req)
        .await
        .expect("send request");
