...
/// leader here; leader_hint=none" election-wait blip on the read path —
/// see each helper's own doc for its exact retryable-status contract).
/// This is NOT "how long to wait before assuming a real bug": every one of
/// these helpers already fails immediately, zero retries, on any
/// status/message outside its own documented allowlist — a genuine error
/// never touches this budget at all (docs/engineering-lessons.md's Testing
/// section: "a retryable, self-resolving condition should not exhaust the
/// same budget as a real error").
///
/// Sized against what a cutover can genuinely take under load, not the
/// steady-state "sub-second" figure ADR 0050 documents — a prior fixed 20s
/// ceiling was observed exhausted on a loaded CI runner during a cascade of
/// overlapping splits even though the cluster was making genuine,
/// uninterrupted progress the whole time. 90s is generous against this
/// file's own outer per-test `tokio::time::timeout` budgets (180-300s)
/// without being unbounded — a call still retrying past this ceiling means
/// the condition genuinely never cleared, which is the one case these
/// helpers must still fail loudly for.
