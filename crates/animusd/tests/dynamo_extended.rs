...
//!
//! `create_table_query_and_conditional_writes` stays here because it
//! exercises wire-level `CreateTable` (`TableStatus`, `ResourceInUseException`
//! on a duplicate) — `dynamo::dispatch_item_op` (the generic core
//! `SimCluster` drives) does not cover `CreateTable` (ADR 0061 rung D3 PR 1).
//! Its sibling test, `concurrent_conditional_puts_one_wins`, moved to
//! `crates/animusd/src/sim_cluster_dynamo_extended.rs` — see that module's
//! own doc for why it needed no `CreateTable` at all.
