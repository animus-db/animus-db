...
//!
//! **ADR 0061 rung H, C-08 PR 4**: three of this file's original four tests
//! converted to deterministic `SimCluster` siblings in
//! `crates/animusd/src/sim_cluster_console_stream.rs` (`table_with_no_
//! stream_reports_the_honest_disabled_answer`, `stream_enabled_lists_
//! shards_and_records_reflect_real_writes`, `walking_a_shard_with_next_
//! shard_iterator_visits_every_record_exactly_once`) and were removed from
//! here. The one remaining test below stays `ProdEnv`.
...
///
/// **KEPT `ProdEnv` (ADR 0061 rung H, C-08 PR 4)**: no primitive drives
/// `animusd::ttl_reaper::ttl_reaper_loop` under `SimEnv` —
/// `SimCluster::new`/`restart` never spawn it — so there is nothing in
/// that fixture that would ever reap the expired item this test depends
/// on (the TTL reaper is its own unowned residual group).
