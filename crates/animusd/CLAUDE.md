...
dynamo_throttling.rs` is now down to its **one** genuinely sim-unreachable
residual, `cluster_wide_throttle_default_is_overridden_by_a_tables_own_
throughput` — a cluster started with the `cluster_settings` config surface
...
override is not. It stays `ProdEnv` because its own subject is
`run_node_with_cluster_settings`'s **config-parse** path itself (a
...
real `animusd` binary would parse, so there is no sim analog. Every other
test this file used to hold has moved to `SimCluster`: `ADR 0061 rung D3`
moved the single-item write/read throttling-and-recovery pair to
`sim_cluster_throttle.rs`'s `write_admits_a_burst_then_refuses_then_
recovers_after_a_full_refill`/`read_admits_a_burst_then_refuses_then_
recovers_after_a_full_refill`; PR 2b moved the five config-surface tests
just described to `sim_cluster_dynamo_update_table.rs` above (see that
module's own doc for the exact list); rung K (C-11, closed) moved
`BatchWriteItem`/`BatchGetItem` shedding, `TransactWriteItems`'
`ThrottlingError` cancellation reason, and the forwarded-write check to
`sim_cluster_dynamo_throttle.rs`, and the counter-assertion test itself,
`admin_metrics_reports_nonzero_throttled_counters`, to
`sim_cluster_admin.rs`'s own scenario (9) — `ThrottledWrites`/
`ThrottledReads`, which this file's and `sim_cluster_dynamo_update_
table.rs`'s own module docs used to claim never increment under
`SimCluster`, a claim stale since D2 PR 1 gave every node a real
`DataRole`, simply unexercised by any fixture until rung K proved it
false. See this file's own consolidated "throttle-metric counters under
SimCluster" appendix below for the full account.
...
**Per-node roles (ADR 0061 rung L, C-12, closed).** `SimCluster` has one node
...
data-role-gated — W-10/ADR 0043 §A9).

**`NodeRole::Data` is first-class at construction since C-12 PR 3** —
`new_with_roles`'s own blanket "every node must run the control plane"
assert is gone, replaced by two narrower ones: at least one node must be
control-bearing (`Control`/`Both`), and `roles` must be **control-prefixed**
— every `Control`/`Both` entry before every `Data` one, so the control
voter set is always node ids `0..control_count` and `self.controls` (one
local `RaftNode<SimEnv>` per control-bearing node) stays a contiguous
prefix. A `NodeRole::Data` node is built exactly like [`SimCluster::grow`]
already builds its own grown one — the two constructors share the same
per-node loop set, factored so grow's own janitor gating stays correct
without duplicating it: `ControlHandle::Remote(RemoteControlClient::new(
control_seeds, ..))` dialing the cluster's own control-bearing node ids, a
real `host::Reconciler`, a `heartbeat_loop` targeting the control-bearing
prefix (never the whole node set — see the `all_ids`→`control_ids` fix
below), a `ttl_reaper_loop`, and `spawn_remote_mirror_sync_loop` (`SimCluster
::grow`'s own `SimEnv`-native long-poll mirror, unchanged). **Deliberately
never spawned on a `NodeRole::Data` node**: `backup_janitor_loop`/
`segment_janitor_loop`/`index_backfill_loop` — control-plane-leader-only,
and a `Remote` control handle can structurally never become control-plane
leader; verified directly against `BoundDataNode::start_data_with_growth`
(`lib.rs`), which spawns none of the three either. **This also fixed two
real, previously-latent gaps in `SimCluster::grow` itself**, found and
closed while factoring the shared construction path: `grow` used to spawn
`backup_janitor_loop` unconditionally on its own grown node (a harmless but
production-inaccurate permanent no-op) and never spawned `ttl_reaper_loop`
at all (production does, on every data-only node) — both fixed to match
`new_with_roles`'s own `NodeRole::Data` construction exactly; `grow`'s own
`segment_store` also switched from an inert per-node `Fs` placeholder to
the same shared `SimSegmentStore` every other node's `ClientCtx::
segment_store` already wraps.

**`SimCluster::restart` now dispatches purely on `node < self.controls.
len()`** (the control-bearing prefix's own length) instead of panicking
above that boundary: a node below it restarts control-bearing (`Local`
handle, unchanged from PR 2 apart from one fix below); a node at or above
it restarts data-only — whether it was minted `NodeRole::Data` at
construction or added later via `SimCluster::grow`, the two are
indistinguishable to this method, and both get a fresh `ControlHandle::
Remote`, a fresh mirror-sync loop, and every data-role loop it actually
owns (reconciler, `heartbeat_loop`, `ttl_reaper_loop` — never the three
control-plane-leader-only janitors, for the identical reason construction
skips them). **A real, previously-latent bug found and fixed along the
way**: a restarted control-bearing node's fresh `RaftNode` now takes
`control_ids` (the real control voter set) as its own membership, not
`0..self.nodes` — the old `all_ids` binding was harmless only as long as
every node ran the control plane, which stopped being guaranteed the
instant any `NodeRole::Data` node exists (at construction, since PR 3, or
via a pre-existing `grow` call); never previously exercised (no
`sim_cluster_growth.rs` scenario ever combined `grow` with a `restart` of
an original node). `SimCluster::crash` needed **no** change at all — it was
already fully role-agnostic (`self.sim.crash(nid(node))` mutes by node id,
independent of `self.controls`' own bounds or what tasks that id runs), and
`Drop for SimCluster`/`Simulator::shutdown` already tear down every node's
tasks — mirror-sync loop included — uniformly regardless of role; no new
`Weak`-handle/Drop-coverage extension was needed.

**Routing and DDL at a data-only node both needed no new code — verified by
tracing, not merely asserted.** `ClientCtx::resolve_cp_route`
(`forwarding.rs:162`) reads only `self.edge` and, on its fallback path,
`Metadata` — neither depends on `self.control` being `Local` or `Remote` —
so a `SimCluster` op issued from a data-only node's index forwards over the
real `SimRelayClient` wire exactly like any other non-replica-hosting
node's op already does, the identical "zero new rejection code" degrade
control-only nodes already established. `ClientCtx::propose_schema`
(`schema.rs:133`) is already fully `<E: Env, R: RelayClient>`-generic, and
its `Remote`-handle fallback tiers (an intra leader-address hint, then a
bounded broadcast over every known intra address) reach a control-bearing
node's own relay server the same way a genuine data-only deployment's join
dance would; `dynamo::create_table` (`dynamo.rs:4099`) never calls
`ctx.data()` at all, so DDL issued at a data-only node's own index commits
through its `Remote` handle with no widening needed either.

**`crates/animusd/src/sim_cluster_data_only.rs` is this PR's own test
module** (5 scenarios, pinned-seed + `_over_seeds`) — see its module doc
for the full account, including the two production ground-truth
corrections its own doc records against this rung's original plan
(`backup_janitor_loop`/`segment_janitor_loop`/`index_backfill_loop` do
**not** respawn on a data-only node's restart — verified against
`BoundDataNode::start_data_with_growth`, which spawns none of the three
either). **Still deferred, deliberately, by this PR**: the real ADR 0030/
0032 join dance — every data-only node here, constructed or grown, still
self-registers via the same direct control-plane-Raft-bypass propose
`SimCluster::seed_members`/`SimCluster::grow` already use, never
`ClientCtx::admin_add_member`'s real relay + failure-detector-promotion
dance; and a `"combined"` (new control-plane voter) growth shape, which
would need `self.controls` itself to grow — `SimCluster::grow` still
supports `role = "data"` only.

`crates/animusd/src/sim_cluster_control_only.rs` is C-12 PR 2's own test
module (5 scenarios, pinned-seed + `_over_seeds`) — see its module doc for
the full account, including the deterministic "force a control-only node
into the control-leader seat" trick scenario (c) needs (a bounded
crash/heal loop, since which node the control group happens to elect first
is seed-dependent). `SimCluster::role_of(node)` is the test-reachable
accessor both PRs' scenarios use.

**C-12 is closed** (2026-09-09, PRs #806/#808/#822/#823/#824/#825/#826 plus
a docs close-out) — see this file's own consolidated "control/data role
split under `SimCluster`" appendix below for the real-socket conversion
record, and ADR 0061's "Rung L closed" amendment for the full per-PR
...
**C-14 (rung N) is closed** (2026-09-14, PRs 1-5) — `SimCluster` had no
way to add a genuinely new `RaftNode<SimEnv>` to the LIVE control-plane
voter quorum after construction (`self.controls` only ever grew at
`new_with_roles` construction time; `SimCluster::grow` only ever added a
**data**-role node). It now has one, `SimCluster::grow_control()`, plus a
combined-role composition over it, `SimCluster::grow_combined()`
(`grow("combined")`) — both admitting a fresh standalone `RaftNode<
SimEnv>` through the real `POST /admin/control/member/add` route, never a
control-plane bypass propose. Of the corrected four-consumer inventory
PR 1's own grep found, two are converted/implemented, one is deferred
with a written verdict (the seed/join dial's `NodeRole::Control` arm —
production's own `--seed`/`join` dial never produces a control voter at
all, so there is no real behavior for the arm to model), and one was
already, and remains, permanently real-socket. See this file's own
consolidated "combined control-plane voter growth under `SimCluster`"
appendix below for the fixture surface, the final residue table, and ADR
0061's "Rung N (post-C-13)" opener amendment through "Rung N closed" for
the full per-PR account, the mechanism summary, and the two gotchas PR
2/PR 4's own test authorship found (a stale crashed-leader `is_leader()`
belief; a propose-then-admit virtual-time race).
...
role split (5/21, **owned and closed** by C-12/ADR 0061 rung L as of
2026-09-09 — this figure is superseded there by the actual grep-verified
outcome: 22 scenarios/43 tests converted across 7 real-socket files by
...
see `docs/roadmap.md`'s C-12 entry), reconciler-driven split/rebalance/GC
(7/13), TTL
...
residual, `cluster_wide_throttle_default_is_overridden_by_a_tables_own_
throughput`, a config-parse-path/process-boundary test with no
`SimCluster` analog; the other five moved to `sim_cluster_dynamo_
throttle.rs`/`sim_cluster_admin.rs` — see this file's own consolidated
"throttle-metric counters under SimCluster" appendix), auto-split loops
(2/2), `--config` bring-up (2/2, **corrected by C-12's close-out**: only
`config_node_identity.rs` genuinely belongs here, and even its own true
reason is not literal `--config FILE` parsing but pinning the real
`Node::bind`/`bind_control`/`bind_data` identity-derivation path, which
`SimCluster` structurally bypasses; the other half of this pairing,
`control_membership_admin.rs`'s own residue test, turned out convertible
on inspection — no `--config` parsing in it either — and was converted
...
throttle metric counters, and the control/data role split are now owned
and closed (C-07, C-08, C-09, C-11, C-12 — see `docs/roadmap.md`'s C-12
entry and this file's own consolidated "throttle-metric counters under
SimCluster" and "control/data role split under SimCluster" appendices
above/below);
index DDL beyond plain `CreateTable` is **owned and closed** by C-10/ADR
0061 rung J as of 2026-09-09 (PRs 1-7 — docs opener; groundwork; `sim_
cluster_dynamo_update_table_index.rs` converting `update_table_create_
index.rs`/`update_table_drop_index.rs`/`dynamo_gsi_drain.rs` (9 tests, all
three deleted whole); `sim_cluster_backfill_seeder.rs` converting 4 of
`backfill_seeder.rs`'s 5 scenarios; `sim_cluster_stream_backfill_seed_
filter.rs` converting `stream_backfill_seed_filter.rs`'s 2 tests, file
deleted whole; `sim_cluster_console_table_config.rs`'s 3 GSI-DDL scenarios
converted from `console_table_config.rs`, file trimmed to its 1 out-of-scope
...
tests stayed `ProdEnv` for documented reasons spread across seven files, as
...
**TTL — the smallest group, and the one C-08's own close-out recommended
first — is closed by C-09 (landed 2026-09-08/09, ADR 0061's "Rung I"/"Rung
I closed" amendments, `docs/roadmap.md`).** See the "TTL reaper under
`SimCluster`" appendix below for the full mechanism and residual account;
in short, `animusd::ttl_reaper::ttl_reaper_loop`'s thin
wrapper and `impl TtlReaperProgressHost for ClientCtx` were widened to
`<E: Env, R: RelayClient>`, `SimCluster` gained an always-on per-node
reaper spawn (200ms sim interval, mirroring the backup/segment janitors'
own precedent) plus `drive_ttl_sweep`, and `dynamo::dispatch_item_op`
gained a `DescribeTimeToLive` arm to close the one dispatch gap the
conversion surfaced. `tests/dynamo_ttl.rs` went from 9 tests to 1
(`expired_item_is_still_readable_immediately`, permanently `ProdEnv` — the
fixture's fixed 12s per-op budget outruns even a single wire call against
the always-on 200ms reaper); `admin_endpoint.rs`'s TTL reaper-progress
test converted (9 tests remain, none TTL-related); `console_stream.rs`'s
sole test converted and the file deleted whole. `cargo test -p animusd
--lib sim_cluster` ran 406 → 428 passed across the rung's five PRs.
...
## Appendix — `sim_cluster_admin_actions.rs` siblings (ADR 0061 rung H, C-08 PR 6, 2026-09-08)

Closes PR 6's own scope: the admin HTTP-JSON interface's **mutating**
actions, `sim_cluster_admin.rs` (PR 5)'s sibling for `POST /admin/
data/dynamo`, `/admin/data/drop-table`, `/admin/data/seed`, `/admin/
tablet/split`, `/admin/credentials`(`/rotate`/`/revoke`), and `/admin/
control/transfer`. Converts 8 of `admin_endpoint.rs`'s remaining 12
tests into a new `crates/animusd/src/sim_cluster_admin_actions.rs` (8
scenarios × pinned-seed + 5-seed `_over_seeds` = 16 tests). **No
`admin.rs`/`dynamo.rs` dispatch change was needed** — every route this
PR drives was already reachable through `GenericAdminHost` (PR 2's own
newtype), the identical seam PR 5 already established.

**One real, previously-latent seam bug found and fixed, this rung's
second (PR 5's own shared `MetricsHandle::noop()` finding was the
first)**: `ClientCtx::admin_transfer_control_leadership` (`lib.rs`)
already had a fully generic `<E: Env, R: RelayClient>` signature (rung
C5) — its own commit-wait loop, though, still read `tokio::time::
Instant::now()`/called `tokio::time::sleep(..)` directly instead of
`self.env.now()`/`self.env.sleep(..)`. A generic *signature* proves
nothing about whether a function's *body* actually avoids the real
clock/timer — the identical lesson ADR 0061 rung G, C-07 PR 2's `index_
drain::seal_now` finding already recorded, and rung F/G's own `recovery_
grace_now_ms`/`txn_recover` findings before that; this is that lesson's
**third** recurrence in this crate, recorded as such (not a new entry)
in `docs/engineering-lessons.md`. `SimEnv` has no real Tokio reactor, so
`tokio::time::sleep` panics ("there is no reactor running") the instant
this loop's first poll iteration is reached whenever the initial arm
attempt doesn't resolve on its very first pass — found immediately by
this PR's own `control_transfer_moves_leadership_to_the_named_node`
scenario, its first real exercise of the route. Fixed with the same
`self.env.now().saturating_add(..)`/`self.env.now() >= deadline`/
`self.env.sleep(..)` conversion every prior rung's own `tokio::time`
finding used — the method's documented behavioral contract (issue
#688's fixed target-must-be-the-observed-leader semantics) is unchanged.
...
| `admin_seed_writes_synthetic_keys` | Converted → `sim_cluster_admin_actions.rs::seed_writes_synthetic_keys` — the full bulk-seed contract; `percent_encode` moved with its last caller |
...
| `admin_control_transfer_moves_leadership_to_the_named_node` | Converted → `sim_cluster_admin_actions.rs::control_transfer_moves_leadership_to_the_named_node` — the scenario that found the seam bug above |
...
| `seed_load_does_not_storm_cp_elections` | **KEPT** — real-thread election-timing liveness |
| `admin_system_table_split_lineage_after_a_real_split` | **KEPT** — `ctx.control_storage` is always `None` under `SimCluster` |
| `admin_storage_compact_action` | **KEPT** — `CpGroup::compact_now()` is `None` for the `MemoryEngine` backend |

**Before/after counts**: `admin_endpoint.rs`: 18 → 10 (8 removed, plus
two now-dead helpers deleted — `bring_up_with_streams_quiesce`,
`percent_encode`, both losing their last caller). `sim_cluster_admin_
actions.rs`: 16 new tests (8 scenarios).
...
seed and every `_over_seeds` seed on the first full run once that fix
landed — no scenario-authoring bug needed a redesign this PR.

**Gates, in the required order**: `cargo build -p animusd --all-targets`
(clean; checkpoint push before the gates below); `cargo test -p animusd
--test admin_endpoint` on the untrimmed file (18 passed, 10.64s); `cargo
test -p animusd --lib sim_cluster_admin -- --test-threads=2` (30 passed,
0 failed — 14 from PR 5 + 16 from this PR, both reached by the unchanged
...
warnings); `cargo test -p animusd --lib sim_cluster -- --test-threads=2`
(383 passed, 0 failed, 2 ignored, 753.07s — 367 baseline + this PR's 16
new tests; anchored-sampler RSS, sampled every 10s from the test
binary's own `/proc/<pid>/status` `VmRSS`: first ~66 MB, peak ~894 MB,
last ~171 MB, well under the 6 GB/30 min budget and consistent with
every prior rung's own no-leak trajectory); `cargo test -p animusd
--test dashboard_endpoint dashboard_u05_control_member_actions` (1
passed, 0.37s — the one other real caller of `/admin/control/transfer`
found by grepping `tests/` for "transfer"; no dedicated `admin_control_
transfer.rs` file exists); `cargo fmt --all --check` (one pass needed —
`sim_cluster_admin_actions.rs`'s own long call sites, applied via
`cargo fmt --all`, then clean); `cargo clippy -p animusd --all-targets
--all-features -- -D warnings` (one fix needed — a `nonminimal_bool`
lint on the drop-table convergence predicate, rewritten per clippy's own
suggestion; clean after). `Cargo.lock` unchanged.

See ADR 0061's "Rung H, PR 6 landed" amendment and `docs/roadmap.md`'s
C-08 entry for the full record.

## Appendix — `sim_cluster_dashboard.rs` siblings (ADR 0061 rung H, C-08 PR 7, 2026-09-08)
...
dashboard.rs` (12 scenarios, 23 tests with `_over_seeds`), and
`dashboard_endpoint.rs` is trimmed to its remaining 4. `tests/console_
endpoint.rs` needed no change — its own 3 tests already reached their
final `ProdEnv` disposition in PR 3, and this PR's own read confirmed
nothing there became sim-convertible in the interim. **Landed ahead of
PR 6 in the series' own numeric order**: PR 6 (admin mutating actions) and
this PR are independent surface — this PR depends only on PR 2's
groundwork (`GenericAdminHost`, `SimCluster::admin`), not on anything
PR 6 touches. **No `admin.rs`/`console.rs`/`dynamo.rs`/`sim_cluster.rs`
change was needed** — every generic handler this PR's scenarios reach was
already built by PR 2 and widened by PR 2 (`update_time_to_live`/`create_
backup`/`delete_backup`), PR 2a/2b (`dispatch_table_op`'s `CreateTable`/
`UpdateTable`-throughput arms), and rung G/C-07 PR 2 (`CreateTable`'s own
GSI/LSI/stream acceptance) — confirmed by direct inspection of `dispatch_
table_op`'s `CreateTable` arm (`dynamo.rs`), not assumed.

**The render-marker half of every converted test reads the served
assets' own compile-time constants directly** — `crate::dashboard::{HTML,
CORE_JS, OVERVIEW_JS, TABLETS_JS, TXNS_JS, STORAGE_JS, BACKUPS_JS,
BROWSER_JS, NODE_JS}` — rather than fetching them over a socket.
`animus_node::admin`'s own module doc says the dashboard's static assets
(the shell HTML, every per-view `.js`) are served from `animusd`'s own
`handle_conn` **before** its `AdminHost` dispatch table is ever reached;
`SimCluster::admin` calls straight into `animus_node::admin::dispatch`
(through `GenericAdminHost`), never `handle_conn`, so it cannot fetch
`/admin/ui/*` at all. Since every one of these assets is a plain
`include_str!` compile-time constant with no request-time computation
whatsoever, reading the constant directly is not a narrower proof than
fetching it over a socket would have been — the exact same bytes, minus a
serving mechanism this rung has no reason to reproduce (`docs/
engineering-lessons.md`'s new matching entry). Each converted test's own
**live JSON round trip** (`/admin/txns`, `/admin/backups`, `/admin/
restores`, `/admin/status`, `/admin/control/members`, the four U-07
observability routes, and two "route exists" probes) goes through
`SimCluster::admin` instead, issued from a **control follower** for a
cluster-wide route (`sim_cluster_console::control_leader_and_follower`)
or a table's own tablet **non-leader** (`non_leader_of_table`) once a
table exists — reusing PR 3's shared helpers exactly as `sim_cluster_
admin.rs`/`sim_cluster_console_stream.rs`/`sim_cluster_console_table_
config.rs` already do, rather than duplicating them.

**Test-by-test disposition**:

| Real-socket test (`dashboard_endpoint.rs`) | Disposition |
|---|---|
| `dashboard_u01_render_only_fixes` | Converted → `sim_cluster_dashboard.rs::u01_render_only_fixes` — Transactions-tab markers + live `/admin/txns`; tablet-detail Raft-field markers; `believes_alive` marker; sparkline markers + live `/admin/metrics/history`; 16-`EntityKind` round trip |
...
| `dashboard_u04_ttl_row` | Converted → `u04_ttl_row` — `CreateTable`/`UpdateTimeToLive` (enable+disable) via the `/admin/data/dynamo` proxy, checked against `/admin/status` |
| `dashboard_u04_create_table_form` | Converted → `u04_create_table_form` — `CreateTable` with a GSI+LSI+stream, then `UpdateTimeToLive`, checked against `/admin/status` |
| `dashboard_u05_control_members_panel` | Converted → `u05_control_members_panel`, issued from a control follower, converged-or-timeout on `/admin/control/members` |
| `dashboard_u05_tablet_actions` | Converted → `u05_tablet_actions` — a pure marker check against `TABLETS_JS`; touches no `SimCluster`, carries no seed (the four routes' own wire-level coverage is `admin_endpoint.rs`'s, PR 6's own territory) |
...
framing/role-split); PR 4's `console_stream.rs` (1, no TTL-reaper-loop
primitive under `SimEnv`) and `console_table_config.rs` (4: three GSI-DDL
blocker-(d) tests + the PITR-present test, `UpdateContinuousBackups` has
no generic dispatch arm); PR 5's `admin_endpoint.rs` (6: auth-config,
`LsmEngine`-only SSTable-counter, `cluster`/`fs`-kind segment-store
placement ×2, genuinely-leaderless-node, and the TTL reaper's own
progress half) plus `system_table.rs` (2, `ctx.control_storage` always
`None`) and `metrics_endpoint.rs` (1, the raw-text `/metrics` listener has
no `SimCluster` analog — a JSON-route analog exists instead); PR 6's own
4-test `admin_endpoint.rs` residue (`admin_interface_surfaces_state_and_
actions` — its one remaining action, `/admin/storage/flush`, has the
identical `MemoryEngine`-has-no-LSM-concept gap PR 5's own `admin_raftkv_
default_does_not_materialize_the_dataset` names; `seed_load_does_not_
storm_cp_elections` — real-thread election-timing liveness; `admin_
system_table_split_lineage_after_a_real_split` — `ctx.control_storage`
always `None`; `admin_storage_compact_action` — `CpGroup::compact_now()`
is `None` for the `MemoryEngine` backend — the other 8 of PR 6's own 12
tests converted into `sim_cluster_admin_actions.rs`, `admin_endpoint.rs`
going 18 → 10 there); and PR 7's own `dashboard_endpoint.rs` (4, listed in
...
6's own landed commit (`02b66fd4`) — four textual doc conflicts (this
file, `lib.rs`, the ADR, `docs/roadmap.md`), each resolved by keeping
both PRs' additions in sequence, no `<<<<<<<`/`=======`/`>>>>>>>` markers
left behind — and ran every gate below for real, in the main tree.
**No product/scenario-authoring bug was found**: every one of the 23
`sim_cluster_dashboard.rs` tests passed at its pinned seed and every
`_over_seeds` seed on the very first run, with no fixture change and no
scenario rewrite needed — the design Phase A committed without being
able to compile it held up unchanged.
...
## Appendix — TTL reaper under SimCluster, C-09 closed (ADR 0061 rung I, PRs 1-6, 2026-09-08/09)

Folds the rung's own five PR-by-PR accounts (previously five separate
appendices) into one coherent record now that the rung is closed — the
mechanism facts and the final residual state, not a chronological replay
of every intermediate revert. See ADR 0061's "Rung I" amendments (PR 2
through "Rung I closed") for the full PR-by-PR narrative, including the
two scenarios PR 3 wrote then reverted before PR 5 restored them.

**Goal.** Make the DynamoDB-style TTL reaper (ADR 0051) — previously
...
seed-replayable under `SimCluster`/`SimEnv`.

**Mechanism, as it stands after the rung.**

- `animus_node::ttl_reaper::{ttl_reaper_loop, ttl_sweep_one_tablet}`
  (`crates/animus-node/src/ttl_reaper.rs`) was already `<E: Env, H:
  TtlScanHost + TtlReaperProgressHost>`-generic before this rung (rung
  C2) — it reads `TtlSpec` via `host.ttl_metadata()`, compares against
  `env.wall_now()` (ADR 0051 §1, the one place `wall_now()` exists for),
  deletes through `host.ttl_delete_if_attribute_equals` →
  `dynamo::kind_write_item_at_leader` (ADR 0049's kind-write path, so
  index/stream/change-log maintenance is inherited, never reimplemented),
  and publishes a `TtlReaperProgress` per phase transition.
  `ttl_sweep_one_tablet` was widened from private to `pub` (PR 2, a pure
  visibility change) so `SimCluster::drive_ttl_sweep` can drive the exact
  per-tablet sweep function the always-on loop calls every tick, rather
  than reimplementing the scan/expire/delete control flow a second time.
- The one remaining concrete surface, `crates/animusd/src/ttl_reaper.rs`'s
  thin wrapper (`pub(crate) async fn ttl_reaper_loop(ctx: ClientCtx,
  interval: Duration)`) and `client_ctx_host.rs`'s `impl
  TtlReaperProgressHost for ClientCtx` (a two-line mutex-update body),
  were widened to `<E: Env, R: RelayClient>` (PR 2) — a pure free-function/
  trait-impl widening, not the riskier trait-impl-on-`ClientCtx` shape C-08
  PR 2 warns against, since neither is ever called through a `dyn`
  production dispatch table. Every production spawn site (`lib.rs`'s five
  `tokio::spawn(ttl_reaper::ttl_reaper_loop(ctx.clone(), ..))` calls) keeps
  passing a bare `ctx.clone()` with zero call-site changes, inferring `E =
  ProdEnv, R = AnimusdRelayClient` from `ClientCtx`'s own definition-site
  default — confirmed, not assumed, by `tests/dynamo_ttl.rs`'s untrimmed
  suite staying green before and after (PR 2). Production's own default
  sweep cadence, `DEFAULT_TTL_SWEEP_INTERVAL` (60s), is unchanged.
- **`SimCluster` spawns the reaper always-on, on every node, unconditionally**
  (`sim_cluster.rs`, PR 2): `SIM_TTL_SWEEP_INTERVAL` (`Duration::
  from_millis(200)`) joins the janitors' own interval constants;
  `ttl_reaper::ttl_reaper_loop` is spawned in both `SimCluster::new` and
  `::restart`, mirroring the backup/segment janitors' own always-on spawns
  exactly — the loop's own per-tablet leader gate (`TtlScanHost::
  ...
  way `auto_split_loop` is. `SimCluster`'s existing `Drop` (issue #753)
  needed no new code to cover this new perpetual loop — the same
  no-new-Drop-coverage finding Rung H's own opener made, restated here
  because this rung's primitive genuinely is a new perpetual loop.
- **`SimCluster::drive_ttl_sweep(node)`** (PR 2) runs one full sweep to
  exhaustion over `node`'s own led tablets, through `ttl_sweep_one_tablet`,
  with its own driver-local cursor entirely separate from the always-on
  loop's — for a scenario that must assert an intermediate, pre-cadence
  reaper state without waiting out the 200ms cadence. **`ttl_reaper_
  progress(node)`** mirrors `backup_janitor_progress`/`segment_janitor_
  progress` exactly (a plain lock/clone/drop, never held across an
  `.await`).
- **`SimEnv::wall_now()` determinism**: `Clock::wall_now` in `animus-sim`
  (`UnixMillis(SIM_WALL_EPOCH_MS.saturating_add(self.now().0 /
  1_000_000))`) is a pure function of virtual time — a `PutItem` at
  `wall_now_secs + delta`, then `run_for`/`drive_*` advancing virtual time
  past `delta`, deterministically expires an item with no real-clock
  dependency at all. No `HashMap`/`Instant::now`/`tokio::spawn`/
  `tokio::time` was found in either widened body (PR 2) — unlike a few
  `tokio::time`-body findings this same template caught in sibling rungs
  (`index_drain::seal_now`, C-07 PR 2; `ClientCtx::
  admin_transfer_control_leadership`, C-08 PR 6), confirmed by actually
  running the loop under `SimEnv`, not just by reading the source.
- **The 12s `OP_BUDGET` caveat.** Every `SimCluster` wire call's
  `spawn_and_capture` drives `self.sim.run_for(OP_BUDGET)` (12s)
  unconditionally, and `Simulator::run_until` always drains every
  scheduled event up to that deadline before returning, regardless of how
  quickly the awaited future itself resolved. With the always-on reaper
  ticking every 200ms, a single wire call already spans 60 sweep
  opportunities — so any assertion that needs to *force* an intermediate
  sweep state (mid-cursor, one-tick-before-reap) must use `drive_ttl_
  sweep`/a short `run_for`, never an ordinary op call (this rung's own
  scenario (b) in `sim_cluster_ttl.rs`), and any assertion that needs to
  *suppress* the reaper for even one wire round trip cannot be reproduced
  through this fixture at all — no "hold the reaper back between two wire
  calls" primitive exists (`drive_ttl_sweep` only ever adds sweeps on top
  of whatever the always-on loop already ran). The latter is exactly
  `expired_item_is_still_readable_immediately`'s own residual reason,
  below.
- **The `DescribeTimeToLive` dispatch gap (PR 3 found, PR 5 closed).**
  `dynamo::dispatch_item_op` — the generic (`SimEnv`-capable) dispatch
  path `SimClusterHandle::dynamo` calls through — had an arm for
  `UpdateTimeToLive` (since ADR 0061 rung H, C-08 PR 2) but none for its
  read-side sibling `DescribeTimeToLive`, so every `DescribeTimeToLive`
  call under `SimCluster` answered `500` ("this operation is not yet
  supported by the generic dispatch path") regardless of request
  validity — an asymmetry invisible from either function's own signature
  (both looked equally "already generic"), surfaced only once PR 3
  actually issued the call. PR 5 widened `dynamo::describe_time_to_live`
  to `<E: Env, R: RelayClient>` (a pure signature change — it already took
  an unused `_ctx: &ClientCtx` and isn't even `async`) and added `Operation::
  DescribeTimeToLive { table } => describe_time_to_live(ctx, meta, &table)`
  to `dispatch_item_op`, mirroring `UpdateTimeToLive`'s own precedent
  exactly. `run_operation`'s own arm is untouched, keeping production
  dispatch byte-identical (the D2 PR 1 lesson applied once more).

**New module**: `sim_cluster_ttl.rs` — 9 scenarios (a)-(i) (`_over_seeds`
×5 each), converting all 9 of `tests/dynamo_ttl.rs`'s original tests
except the one permanent residual below: `PutItem`/`UpdateTimeToLive`
issued from node 0 (a schema-catalog mutation, no tablet leader to route
around) or from a **non-leader** of the table's tablet for item writes
(mirroring every `sim_cluster_dynamo_*` sibling's own forwarding-path
convention), reap/no-reap asserted with `ConsistentRead: true` (ADR 0055).
Scenario (i) reuses C-07 PR 3's stream wire shapes as module-private local
copies (not exported across modules). `sim_cluster_admin.rs` gained
scenario (8) (the reaper-progress half of the admin residue, kept separate
from scenario (6)'s narrower tables-only regression). `sim_cluster_
console_stream.rs` gained scenario (4) (the console-side identity walk,
`console_stream.rs` deleted whole — it was the file's only test).

**The final `ProdEnv` residue — exactly one test, one file:**

| File | Kept test | Reason |
|---|---|---|
| `dynamo_ttl.rs` | `expired_item_is_still_readable_immediately` | The `OP_BUDGET`-vs-always-on-reaper gap above: no primitive can suppress the reaper for even one wire round trip, so the "expired but not yet reaped" state (ADR 0051's AWS-faithful visibility — expired items stay readable until reaped, deliberately not filtered) can never be observed through this fixture |

`admin_endpoint.rs` keeps its other 9 tests (none TTL-related — the TTL
reaper-progress test converted). `console_stream.rs` is **deleted**
(nothing left to keep `ProdEnv` for). This is the first rung in the whole
F-through-I sequence to leave only a single genuinely-permanent residual
test, not a whole class.

**Test-count trajectory** (`cargo test -p animusd --lib sim_cluster --
--test-threads=2`, whole tier): 406 (C-08 baseline) → 408 (PR 2, +2) → 420
(PR 3, +12 net — 16 written, 4 reverted with the `DescribeTimeToLive`-
blocked scenarios) → 424 (PR 4, +4) → **428 passed, 0 failed, 2 ignored**
(PR 5, +4). No leak trajectory across any PR (anchored-sampler/`/usr/bin/
time -v` RSS stayed within the same envelope every prior rung confirmed).
`Cargo.lock` unchanged throughout the rung.

**No product bug found** in any PR — the rung's only findings were the
two narrow, already-generic-signature gaps named above (the `OP_BUDGET`
caveat is a fixture-capability boundary, not a bug; the `DescribeTimeToLive`
gap was a real, narrow dispatch-table omission, fixed in PR 5). See ADR
0061's "Rung I closed" amendment for the cross-cutting mechanism lessons
this rung's own findings generalize into.

See ADR 0061's "Rung I" amendments (PR 2 through "Rung I closed") and
`docs/roadmap.md`'s C-09 entry for the full per-PR record.


## Appendix — index DDL beyond plain `CreateTable` under SimCluster, C-10 closed (ADR 0061 rung J, PRs 1-7, 2026-09-09)

Folds the rung's own five PR-by-PR accounts (previously five separate
appendices) into one coherent record now that the rung is closed — the
mechanism facts and the final residual state, not a chronological replay
of every intermediate revert. See ADR 0061's "Rung J" amendments (PR 2
through "Rung J closed") for the full PR-by-PR narrative, including the
two documented sequenced-analogue substitutions in PR 3 and the fixture
gap PR 4 found and fixed.

**Goal.** Make `UpdateTable`'s GSI add/drop half — previously provable only
over `ProdEnv`'s real wire via nine `tests/*.rs` files/33 tests — deterministically
fault-injectable and seed-replayable under `SimCluster`/`SimEnv`.
`dispatch_table_op`'s `UpdateTable` arm rejected any call carrying an index
change outright (`unsupported_by_generic_dispatch`); this was blocker (d),
named by both C-08's and C-09's own close-outs, the largest of the five
groups Rung H's close-out left unowned and the only one of those five with
a rung against it.

**Mechanism, as it stands after the rung.**

- `dynamo.rs`'s `create_index`/`drop_index`/`set_index_status`/
  `drop_table_index` widened to `<E: Env, R: RelayClient>` (`tokio::time::
  Instant::now()`/`sleep` → `ctx.env.now()`/`ctx.env.sleep(..)`) — every
  `ClientCtx` method they call (`propose_schema`/`drop_table_tablets`/
  `clear_backfill_cursor_for_table`) was already generic, so this was pure
  signature/clock-call surgery mirroring `update_table_throughput`'s own
  already-landed conversion, no new capability (PR 2). `dispatch_table_op`'s
  `UpdateTable` arm gained a fourth sub-arm, `(None, Some(update), None)`,
  mirroring its stream/throughput siblings exactly: `wire::IndexUpdate::
  Create` → `create_index`, `Delete` → `drop_index`, then the same
  re-describe-on-success every sibling arm already does. `update_table`/
  `run_operation`/`execute_routed` stayed byte-identical throughout — verified
  by `git diff` and by the untrimmed real-socket suites (20 tests across six
  files) staying identical before conversion.
- `index_drain::drain_tablet`/`reconcile_partition`/`gsi_caught_up` (the GSI
  **drain** side) were already `<E, R>`/`<E, R>`/`<E>`-generic since D3 PR
  3b, unchanged by this rung. `animus_node::index_backfill::
  index_backfill_loop` (the **control-plane-leader-only completion
  aggregator** — flips a fully-reported index from `Creating` to `Active`,
  distinct from the per-tablet seeder below) was already fully generic
  (rung C2) and its host trait (`ControlLeaderHost`) already implemented
  generically for `ClientCtx` — only `crates/animusd/src/index_backfill.rs`'s
  18-line thin wrapper was concrete, the smallest lift in the whole rung
  (PR 2).
- `index_drain::backfill_seed_tick`/`advance_backfill_cursor`/
  `seed_change_log_record` (the per-tablet **backfill seeder**, populating
  `Metadata::index_backfill` by scanning a led tablet's own base rows —
  a distinct loop from the completion aggregator, called only from
  `index_drain::change_consumer_loop`, which `SimCluster` never spawns)
  widened to the `Env` seam the same way (`&CpGroup` → `&CpGroup<E>` via
  `group.env()`, `seal_now`'s own established pattern) (PR 2). **A fourth
  recurrence of the generic-signature-does-not-imply-seam-clean-body
  lesson**: `clear_backfill_cursor<E: Env>` (used by `drop_index`'s cascade)
  already carried a generic signature but its commit-wait loop still called
  bare `tokio::time::Instant::now()`/`tokio::time::sleep`, found by the
  first sim smoke run and fixed with the identical `group.env()` conversion
  — see `docs/engineering-lessons.md`'s matching entry (this crate's fourth
  recurrence of that lesson).
- **`SimCluster::new`/`::restart` spawn `index_backfill::index_backfill_loop`
  unconditionally on every node** (PR 2), mirroring the backup/segment/TTL
  janitors' own always-on spawns — its own leader gate
  (`ControlLeaderHost::control_leader`) already makes a non-leader's tick a
  cheap no-op, so no opt-in gate is warranted. **A real fixture gap found by
  a real gate run, not by inspection** (PR 4): `SimCluster::restart`
  respawned `heartbeat_loop`/`backup_janitor_loop`/`segment_janitor_loop`/
  `ttl_reaper_loop`/(conditionally) `auto_split_loop` — every perpetual
  background task `Simulator::stop` drops for the restarted node — but not
  `index_backfill_loop`, spawned unconditionally since PR 2. Fixed by
  mirroring the `ttl_reaper_loop` respawn immediately above it; a fixture
  `restart` that respawns only *some* of a process's always-on loops
  silently weakens every crash scenario depending on the missing one (see
  Mechanism lessons below).
- **`SimCluster::drive_backfill_seed(node, table)`** (PR 2, new, mirroring
  `drain_gsi`'s exact shape) drives one `index_drain::backfill_seed_tick`
  per `Creating` GSI (never `Active`) over every tablet `node` both hosts
  and leads. A call seeds up to `BACKFILL_SEED_BATCH` (256) newly discovered
  partitions per `(tablet, index)` pair and, only once the sweep reaches the
  tablet's current range end, reports via `MetaCommand::MarkIndexBackfilled`
  — **a populated table with more partitions than that needs several
  calls**, never one call assumed to finish everything; a scenario asserting
  full backfill completion loops `drive_backfill_seed` (plus `drain_gsi` and
  a `DescribeTable`/console poll) to exhaustion, the same "several ticks, not
  one" contract the real per-node loop has in production.
- **The console's `add_gsi`/`drop_gsi` needed zero change.**
  `GenericConsoleBackend::add_gsi`/`drop_gsi` were already generic since
  C-08 PR 2, already routing through `execute_routed_as_generic` into
  `dispatch_table_op` — previously dead-ending in
  `unsupported_by_generic_dispatch`, they now fall through to PR 2's own
  index-change sub-arm and the real `create_index`/`drop_index` with **no
  `lib.rs`/`console.rs`/`dynamo.rs` change at all** (PR 6).

**New modules and mapping.**

- `sim_cluster_index_ddl.rs` (PR 2): 2 scenarios — `UpdateTable` adding a
  GSI to a populated table returns it `CREATING`/`Backfilling: true`,
  rejects a `Query` against it while backfilling, and converges to `ACTIVE`
  via `drive_backfill_seed` + `drain_gsi` (a bounded loop of further
  `DescribeTable` calls, never one call assumed to finish) with the
  expected rows queryable; deleting that GSI leaves it absent from
  `DescribeTable` and rejects a `Query` against the now-gone name.
  `converge_gsi_active`, this module's own bounded-poll helper, is the
  shape every later module's own `converge_gsi_active`-family helper
  duplicates locally (this crate's per-file-fixture convention).
- `sim_cluster_dynamo_update_table_index.rs` (PR 3): converts `tests/
  update_table_create_index.rs` (4), `tests/update_table_drop_index.rs`
  (4), and `tests/dynamo_gsi_drain.rs` (1) — 9 tests, 18 `#[test]` fns
  (pinned seed + `_over_seeds` per scenario), all three real-socket files
  deleted whole. No `dynamo.rs`/`index_drain.rs`/`sim_cluster.rs` change —
  driver-plus-assertions only, using primitives PR 2 already supplied.
  Client-side validation rejections, the `MAX_GSI_PER_TABLE` cap, a
  non-leader-issued `UpdateTable` converging on every node, a
  populated-table drop's catalog + physical reclaim, a drop racing an
  in-flight backfill, a create-drop-recreate cycle proving the backfill
  cursor is genuinely cleared, and the GSI drain itself all convert.
  **Three documented substitutions** (the module's own top-of-file doc
  has the full account): `in_flight_backfill_is_cancelled_by_a_concurrent_
  drop` issues the drop immediately after exactly one partial `drive_
  backfill_seed` tick (300 rows, past the seeder's per-tick cap, so the
  index is provably still `Creating`) rather than racing a live
  `tokio::join!`; `a_crash_and_retry_mid_cascade_still_converges` spawns
  the `Delete` by hand, drives the simulator a few milliseconds, then
  interrupts with `SimCluster::restart` — on a 3-node cluster rather than
  the original's 1-node, since `restart` rebuilds the restarted node's own
  control-plane log from scratch and relies on peer catch-up (a lone node
  has no peer to catch up from); `drop_of_an_active_index_on_a_populated_
  table_reclaims_everything` substitutes a stronger `Metadata`/
  `hosted_tablets`-absence-plus-empty-engine converged-or-timeout poll for
  the original's real-WAL-file-absence check (`MemoryEngine` has no file).
- `sim_cluster_backfill_seeder.rs` (PR 4): converts 4 of `tests/
  backfill_seeder.rs`'s 5 scenarios (ADR 0045 §2) — materializing every
  pre-existing row then flipping `Active`; live writes during backfill
  converging to the correct final GSI (issued *sequenced*, not raced — no
  automatic background sweep to race against in this fixture, proving the
  load-bearing property, not the literal concurrency); two indexes
  creating simultaneously converging independently; a crash-and-restart
  mid-backfill (300 rows exceeding `BACKFILL_SEED_BATCH`, resuming from the
  durable `KIND_CURSOR` row) — each with an `_over_seeds` sibling.
  `split_during_backfill_converges_with_correct_final_gsi` stayed `ProdEnv`,
  exercising the rung opener's own explicit license (see the residue table
  below).
- `sim_cluster_stream_backfill_seed_filter.rs` (PR 5): converts `tests/
  stream_backfill_seed_filter.rs`'s two tests 1:1 (open-tail serve path,
  sealed-shard path), file deleted whole, no residual. No new `SimCluster`
  mechanism needed — built entirely from `drive_backfill_seed` (PR 2) and
  C-07 PR 2's `drive_stream_seal` together. The original's `no_seal_knobs`/
  `tiny_seal_knobs` pair has no sim analogue and needs none (this fixture
  never spawns the periodic seal arm those knobs tune); the original's
  60s real-time convergence loops simplified to direct assertions off one
  deterministic call, since every write is already committed before the
  single seal/lineage-walk call that follows it.
- `sim_cluster_console_table_config.rs` (PR 6): converts the 3 GSI-related
  `console_table_config.rs` scenarios (`add_gsi_rejects_an_unknown_
  attribute_type`, `add_gsi_records_a_declared_attribute_type`,
  `add_and_drop_gsi_round_trip`) as (6)/(7)/(8), each pinned-seed plus
  `_over_seeds` (6 new `#[test]` fns). The first is pure client-side
  validation (never reaches dispatch); the second needs no convergence
  loop; only the third round-trips a populated table's GSI through
  `CREATING` → `ACTIVE`, gaining its own `converge_gsi_active_via_console`
  helper (mirrors `sim_cluster_index_ddl.rs`'s shape, polling the console's
  own `GET /console/api/tables/{name}` rather than `DescribeTable`). No
  `lib.rs`/`console.rs`/`dynamo.rs`/`sim_cluster.rs` change needed — pure
  test authorship on top of PR 2's own product change.

**The final `ProdEnv` residue, in full** (from the rung's own nine-file/
33-test scope; `schema_ddl_relay.rs`'s 7 tests are a separate, deliberately
unclaimed piece of work never in this scope — a real-thread bring-up
proving the aggregator loop directly via `ClientRequest::ProposeSchema`,
not through the DynamoDB wire):
...
| `console_table_config.rs` | `table_detail_shows_pitr_status_and_backups` | `UpdateContinuousBackups` has no generic-dispatch arm and the test needs the real wall-clock-timed `pitr_snapshot_loop` — a separate, pre-existing PITR residual never in this rung's own scope |

16 tests total — one more than the opener's own 15-test prediction, a
stated, pre-licensed deviation (`backfill_seeder.rs`'s own per-file table
already flagged this outcome as possible), not a new blocker. 18 tests
converted across PRs 3-6.

**Test-count trajectory** (`cargo test -p animusd --lib sim_cluster --
--test-threads=2`, whole tier): 428 (C-09 baseline) → 432 (PR 2, +4) → 450
(PR 3, +18) → 458 (PR 4, +8) → 462 (PR 5, +4) → **468 passed, 0 failed, 2
ignored** (PR 6, +6). No leak trajectory across any PR (peak RSS ~958 MB
→ ~970 MB, `/usr/bin/time -v`, consistent with every prior rung's own
envelope). `Cargo.lock` unchanged throughout the rung.

**No product bug found** beyond the two fixed in place (the `clear_
backfill_cursor` clock-body finding, PR 2; the `SimCluster::restart`
missing-respawn finding, PR 4) — both narrow, generalizable fixture/seam
findings, not product defects reachable in production. See ADR 0061's
"Rung J closed" amendment for the cross-cutting mechanism lessons this
rung's own findings generalize into.

See ADR 0061's "Rung J" amendments (PR 2 through "Rung J closed") and
`docs/roadmap.md`'s C-10 entry for the full per-PR record.

## Appendix — throttle-metric counters under SimCluster, C-11 closed (ADR 0061 rung K, PRs 1-4, 2026-09-09)

Folds the rung's own two PR-by-PR accounts (previously two separate
appendices) into one coherent record now that the rung is closed — the
mechanism facts and the final residual state, not a chronological replay.
See ADR 0061's "Rung K (post-C-10)" opener amendment through "Rung K
closed" for the full PR-by-PR narrative, including the exact line-number
corrections PR 1's own grep made against its plan draft.

**Goal.** Make `ThrottledWrites`/`ThrottledReads` (ADR 0065's own
throttling mechanism's observability surface) — previously provable only
over `ProdEnv`'s real wire via one file/6 tests — deterministically
fault-injectable and seed-replayable under `SimCluster`/`SimEnv`. This was
the smallest of the four groups Rung J's own close-out left unowned, and
the group's own documented blocker (the claim that `SimCluster`'s nodes
carry no metrics sink) turned out stale on inspection.

**Mechanism, as it stands after the rung.**

- **Zero production signature changes — this rung's own distinguishing
  property.** Every call site `ThrottledWrites`/`ThrottledReads` pass
  through was already `<E: Env>`/`<E: Env, R: RelayClient>` generic before
  this rung began: `dynamo::kind_write_item_at_leader` (`dynamo.rs:9069`),
  `write_path.rs`'s ADR 0049 fast marker arm (`:327`),
  `txn_coordinator.rs`'s per-action check (`:134`) on the write side; the
  read-path precharge site (`read_path.rs:99`, inside `ClientCtx::
  throttle_precharge_read`); and `dynamo::dispatch_item_op`, which already
  routed `BatchWriteItem`/`BatchGetItem`/`TransactWriteItems` (the last via
  `run_transact`, generic since C-06 PR 2) through the identical generic
  core production uses. This rung was pure test-fixture authorship
  throughout — no `dynamo.rs`/`lib.rs`/`admin.rs` production code touched
  by any PR.
- **The counters *do* increment under `SimCluster`, and have since rung
  D2 PR 1** — `SimCluster::new`/`::restart` build every node with a real
  `DataRole` (`data: Some(DataRole { raftkv_metrics: node_metrics[i].
  clone(), .. })`, `sim_cluster.rs:1460`) and a `ThrottleTracker`
  (`:1504`), so the metric-recording sites' `ctx.data()`/
  `self.data.as_ref()` accesses have resolved to a real sink for three
  rungs. `SimCluster::set_throttle_defaults`/`::set_throttle_defaults_all`
  (`:2121`/`:2134`) and `SimCluster::admin` (`:933`) already existed and
  already drove `GET /admin/metrics`; `metrics_view<E, R>` (`admin.rs:
  1982`) is generic, dispatched from `animus-node/src/admin.rs:52`. The
  counters simply had no `SimCluster` scenario that ever issued a real
  refusal against them until this rung wrote one.
- **The stale claim had three copies; this rung corrected all three.**
  `sim_cluster_throttle.rs`'s and `sim_cluster_dynamo_update_table.rs`'s
  own module docs both asserted the counters "never increment under
  `SimCluster`" because nodes carried `data: None` — corrected in PR 1
  (the opener), once PR 2's own scenario had proven the claim false. A
  third copy of the identical claim, `sim_cluster.rs:1427`'s own
  `AdminInfo` comment ("No `DataRole` on any node in this fixture (`data:
  None` below)"), survived two more PRs because `sim_cluster.rs` sat
  outside PRs 1-3's own allowed-edits list — corrected in PR 4 (this
  close-out), comment-only, no code change.

**New modules and mapping.**

- `sim_cluster_dynamo_throttle.rs` (PR 2, `#[cfg(test)] mod`, registered
  in `lib.rs` beside `sim_cluster_throttle`): converts four of `tests/
  dynamo_throttling.rs`'s six scenarios, same names, each with a
  pinned-seed test plus its `_over_seeds` sibling (8 `#[test]` fns):
  ...
  `a_forwarded_write_is_throttled_on_the_leader`. Every scenario mirrors
  its original's request shapes, table schema, and assertions verbatim
  (`UnprocessedItems`/`UnprocessedKeys` array contents,
  `TransactionCanceledException`'s `ThrottlingError` cancellation reason,
  `ProvisionedThroughputExceededException`'s full `__type` string).
  `batch_write_item_sheds_throttled_rows_into_unprocessed_items` keeps the
  original's **streamed**-table requirement (a plain marker table's
  `BatchWriteItem` commits one `KindBatch` entry per tablet, so only a
  streamed table's per-item evaluate-at-leader funnel gives genuinely
  partial shedding to observe). **One deliberate behavioral difference**:
  `a_forwarded_write_is_throttled_on_the_leader` issues every write from
  ONE fixed non-leader node (via `SimCluster::leader_index_of`) rather
  than the original's round-robin across all three dynamo listeners — a
  more direct proof of the same claim, that the leader's own bucket is
  what refuses regardless of which node the client dialed. `tests/
  dynamo_throttling.rs` stayed byte-identical and untrimmed after this PR
  (the D3 discipline — prove the untrimmed baseline green before removing
  anything).
- `sim_cluster_admin.rs` scenario (9) (PR 3): adds `admin_metrics_
  reports_nonzero_throttled_counters` (pinned-seed plus `_over_seeds`,
  seed prefix `0xC085_0009`/`0xC085_0900`) — the fifth and final
  conversion, the one scenario PR 2 deliberately left unconverted since it
  asserts the counter directly rather than proving the sink is merely
  live. A single-node `SimCluster` (mirroring the original's own
  `bring_up(1, ..)`): `CreateTable`, a seed `PutItem`,
  `SimCluster::set_throttle_defaults_all(Some(1), Some(1))`, a bounded
  (20-iteration) `PutItem` loop draining the write bucket and a bounded
  (40-iteration) `GetItem` loop draining the read bucket, then `GET
  /admin/metrics` on the table's own tablet leader — asserting
  `counters.throttled_writes > 0`, `counters.throttled_reads > 0`, and a
  nonempty per-tablet `throttle` array, verbatim the original's own three
  assertions. Single-node, not three, matching the original's own
  `bring_up(1, ..)` shape (with exactly one node there is exactly one
  candidate bucket to refuse) rather than reaching for the multi-node
  shape PR 2's forwarded-write scenario needed. Adds two small local
  helpers (`big_value`/`put_body`) mirroring `dynamo_throttling.rs`'s/
  `sim_cluster_dynamo_throttle.rs`'s own identically-named copies rather
  than reaching into either sibling module's `pub(crate)` surface — this
  crate's per-file-fixture convention.

**`tests/dynamo_throttling.rs` trimmed in PR 3** to its one remaining
residual — removing the five converted tests and the helpers only they
used (the plain single-node `bring_up`, the raw HTTP/1.0 `admin` client,
`set_throttle_defaults_everywhere`, `create_streamed_table`, `error_type`,
and the now-unused `use serde_json::Value;`). `create_table`,
`create_table_with_throughput`, `big_value`, `put_body`, `dynamo`, and
`await_bootstrap` all stayed — the remaining test uses every one of them.

**The final `ProdEnv` residue, in full:**

| File | Kept tests | Reason |
|---|---|---|
| `dynamo_throttling.rs` | `cluster_wide_throttle_default_is_overridden_by_a_tables_own_throughput` | Targets `run_node_with_cluster_settings`'s **config-parse** path (a node-bring-up/process-boundary shape — `ClusterSettings` read at construction time), not a throttle-bucket check the generic dispatch core reaches; `SimCluster::new` builds every node in-process from its own fixed constructor, never from a `ClusterConfig` a real `animusd` binary would parse, so there is no `SimCluster` analog |

One test — a permanent residual, not a gap, matching PR 1's own
disposition table exactly.

**Test-count trajectory** (`cargo test -p animusd --lib sim_cluster --
--test-threads=2`, whole tier): 468 (C-10 baseline) → 476 (PR 2, +8) →
**478 passed, 0 failed, 2 ignored** (PR 3, +2). No leak trajectory across
either PR (peak RSS ~983 MB → ~950 MB, `/usr/bin/time -v`, consistent with
every prior rung's own envelope). `Cargo.lock` unchanged throughout.

**No product bug found** — every PR's own gate run (PR 2/PR 3 authored
with no `cargo` access, per this crate's own engineering-lessons entry on
verifying `SimCluster` fixture code by reading the production dispatch
path directly; gated in the main tree after rebase) required no fixes
against either module as authored. See ADR 0061's "Rung K closed"
amendment for the cross-cutting mechanism lessons this rung's own
findings generalize into.

See ADR 0061's "Rung K" amendments (opener through "Rung K closed") and
`docs/roadmap.md`'s C-11 entry for the full per-PR record.

## Appendix — control/data role split under SimCluster, C-12 closed (ADR 0061 rung L, PRs 1-5 (plus 4a-4e), 2026-09-09)

Folds this rung's five conversion-PR appendices (previously separate,
PR 4a-4e) into one coherent record now that the rung is closed — the
constructor shapes, what each role runs, and the retry-helper discipline
that generalize past this rung, not a chronological PR-by-PR replay. See
ADR 0061's "Rung L (post-C-11)" opener amendment through "Rung L closed"
for the full per-PR narrative, gate figures, and — the rung's own
distinguishing finding — the assess-and-close record on `control_
membership_admin.rs`'s 12 tests, planned for conversion in the opener but
not delivered until this PR (PR 4e), the rung's final conversion PR.

**New modules and what each converts.** `sim_cluster_control_data_
split.rs` (PR 4a): `control_only.rs`/`data_only.rs`/`cluster_split.rs`'s
11 real-socket tests → 11 scenarios/22 tests; `cluster_split.rs` deleted
whole (zero residual once converted), the other two trimmed to one
bring-up smoke apiece. `sim_cluster_split_cluster.rs` (PR 4b, kept
**separate** from PR 4a's module — folding them grew that file past this
rung's own ~1800-line-per-module guidance, confirmed empirically and
reverted): 6 of `split_cluster.rs`'s 8 tests → 6 scenarios/12 tests, file
trimmed to its two class-B residuals, never deleted. `sim_cluster_
console.rs`/`sim_cluster_dashboard.rs` (PR 4c, extending the existing
modules rather than a new file): the four role-named tests in `console_
endpoint.rs`/`dashboard_endpoint.rs` → 7 new tests (one conversion is
`#[should_panic]`, so carries no `_over_seeds` sibling). `sim_cluster_
stream_janitor.rs` (PR 4d, extending the existing module): `stream_
janitor.rs`'s one role-named test → 2 tests. `sim_cluster_control_
membership_admin.rs` (PR 4e, new module): `control_membership_admin.rs`'s
11 of 12 tests → 11 scenarios/22 tests, file trimmed to its one permanent
`merge_peer` residual.

**Two new `SimCluster` accessors, both reference primitives for future
scenarios**: `control_voters(node) -> Option<BTreeSet<NodeId>>` (PR 4a,
wraps `self.shared.ctx(node).control.config()` — always `Some` for a
control-bearing node, `None` for a `NodeRole::Data` node's `Remote`
handle until at least one `Status`/`WatchMetadata` round trip has
landed) and `control_leader_index_excluding(exclude: u64) -> u64` (PR
4b, a `control_leader_index` sibling that skips a crashed former
leader's own index while polling for the new one). PR 4e needed no new
accessor: every route `control_membership_admin.rs` drives
(`control_members_view`, `action_add_control_member`,
`action_remove_control_member`, `config_view`, `action_add_member`) was
already a trait method on both the concrete and generic `AdminHost`
impls.

**The stale-self-belief gotcha generalizes past the CP-data tablet
leader it was first found for (PR 4b).** A crashed control node is
...
crashing the leader can therefore keep returning that same crashed
node's own index, never noticing the survivors' real election — the
identical gotcha `sim_cluster_auto_split.rs`'s own module doc already
documents for a tablet leader's `leader_index_of`, now proven to apply
to the control plane too. `control_leader_index_excluding` is the fix;
check every leader-index accessor a fault scenario calls after a crash,
not just the one the first bug report names.

**Retry-helper discipline for a converted scenario chaining a fault
directly into a write/DDL call (PR 4b, reused unmodified by PR 4e's own
leader-self-removal transfer-then-retry scenario)** — three post-fault
races found and fixed, none a product bug; the mechanism each scenario
exercises is correct, what needed fixing was giving the fixture-side
scenario the same retry discipline a real client or production caller
already has, since a converted scenario's own single-shot call has no
real-network timing buffer to hide behind:
...
- **DDL issued immediately after a control-plane recovery** can lose the
  ...
  though the schema and tablet DO commit. `create_table_after_recovery`
  tolerates exactly this one failure shape (a `500` naming "did not
  become serveable", or an outright `ResourceInUseException` if a retry
  raced an earlier attempt's own already-committed schema) and retries
  once.
- **A write issued immediately after a simultaneous control+data fault**
  ...
  disruption. `put_retry` retries `SimCluster::put` on any error,
  bounded, advancing virtual time between attempts.
- **A write issued into a tablet's own crossover window** can hit the
  ...
  instant the fork happens, since this fixture never runs the periodic
  cutover driver as a background loop. `put_raw_retry` retries
  `SimCluster::put_raw` while asserting the transient's own substring
  and driving `SimCluster::drive_inplace_split_cutover` on every
  relevant node id each attempt — mirroring `sim_cluster_auto_split.rs`'s
  own `put_item_retry` idiom exactly.

**A real finding, confirmed by running the scenario, not by inspection
(PR 4c).** A console item mutation dispatched against a control-only
node's own `ClientCtx` genuinely panics, inside `dynamo::fast_marker_
write`'s unconditional `ctx.data().request_rates.observe(..)` read on
the ISSUING node, before any routing decision — stronger than the
"forwards cleanly" shape PR 4a's own `mixed_cluster_put_via_control_
node_forwards_to_data_node` precedent suggested for the plain client
protocol. Not a product bug — the exact mechanism `Node::console_addr()`'s
own panic keeps structurally unreachable in production (a control-only
node never binds the console listener at all). Proven by `sim_cluster_
console.rs::console_item_write_from_a_control_only_node_panics`
(`#[should_panic]`, no `_over_seeds` sibling — a seed-looping variant
would only ever exercise its first seed).

**Two scenario-authoring bugs found and fixed (PR 4a), both fixture
mistakes, not product bugs**: a bare 3-control-only cluster can never
provision a wire `CreateTable` at all (no data-capable member,
`await_table_serveable` spins forever) — fixed by widening to
`[Control, Control, Control, Data]`; a crash+put scenario omitted the
`create_table` call `SimCluster::put`'s non-auto-provisioning
`cp_kind_write_raw` needs (unlike production's auto-provisioning
plain-protocol `Put`, via `marker_batch_write_raw`) — fixed by adding it
ahead of the crash.

**One real, previously-latent production bug found and fixed by PR 4e's
own required gate — this crate's fifth recorded recurrence of the
identical lesson** (generic signature ≠ seam-clean body: `index_drain::
seal_now`, C-07 PR 2; `admin_transfer_control_leadership`, C-08 PR 6; a
third named in that PR 6 appendix; a fourth this same PR 4e's own doc
comment names inline): `ClientCtx::admin_remove_control_member`
(`lib.rs`) already carried a fully generic `<E: Env, R: RelayClient>`
signature, but its own leader-self-removal transfer-wait loop still
called `tokio::time::Instant::now()`/`tokio::time::sleep(SCHEMA_POLL_
INTERVAL)` directly instead of `self.env.now()`/`self.env.sleep(..)`.
`SimEnv` has no real Tokio reactor, so the very first poll of that loop
that doesn't resolve on its first pass panicked ("there is no reactor
running") — found immediately by PR 4e's own `remove_control_voter_
refusals_transfer_and_quorum_warnings` scenario, its first real exercise
of the self-removal transfer arm under `SimCluster`. Fixed with the same
`self.env.now().saturating_add(..)`/`self.env.now() >= deadline`/
...
**A previously-mislabelled residual, corrected by PR 4e's own
re-classification, not taken on the opener's word.** `control_
membership_admin.rs`'s 12th test (`admin_config_reports_the_internal_
addr_the_cli_resolves_control_add_through`) was carried forward by the
opener and by this close-out's own first draft as a permanent `--config`
bring-up residual. Reading the test's own current body first (per this
rung's own distinguishing lesson, below) overturned that label: it never
parses a config file at all — `bring_up_combined` builds its
`ClusterConfig` in-process, no file on disk — and only asserts a `GET
/admin/config` JSON-shape fact `SimCluster::admin` already reaches, so it
converts too, along with 10 of the file's other 11 tests. The one test
that stays real-socket for good, `runtime_added_voter_survives_
leadership_change_to_a_different_original_voter`, proves `ProdEnv::
merge_peer`'s own known scope limit (a runtime-added voter's dial
address is only ever merged into whichever node happened to be leader at
add time, until the replicated `NodeAddrs.control` field +
...
that `SimCluster` never overrides, and whose route table is fully seeded
at construction so every node already knows how to dial every other one
regardless of which node added it or when. `grow_control_group_
converges_everywhere` converts as a weakened substitute (remove-then-
re-add an existing control-bearing node, since `SimCluster::grow`
supports data-only growth only) — the ADD mechanism itself is exercised
identically either way; only the fresh-process growth-node bring-up
specifics go unreproduced, and those are inherently `ProdEnv`/
real-socket concerns.

**Final real-socket disposition, all 8 files this rung's own plan
named** (see ADR 0061's "Rung L closed" amendment for the full
before/after table and the class-B one-line reasons quoted from each
kept test's own doc comment):

| File | Before | After | Disposition |
|---|---|---|---|
| `control_only.rs` | 3 | 1 | 2 converted, 1 real bring-up smoke kept |
| `data_only.rs` | 5 | 1 | 4 converted, 1 `control_storage` availability contract kept |
| `cluster_split.rs` | 3 | 0 | deleted whole, zero residual |
| `split_cluster.rs` | 8 | 2 | 6 converted, 2 real-disk/config-boundary residuals kept |
| `console_endpoint.rs` | 3 | 3 | 2 gained sim siblings but stay whole (mixed — real listener/HTTP-framing half); 1 unrelated C-08 original untouched |
| `dashboard_endpoint.rs` | 4 | 3 | 1 converted whole; 1 gained a sim sibling but stays whole (mixed); 2 unrelated residuals untouched |
| `stream_janitor.rs` | 2 | 1 | 1 converted; 1 unrelated C-07 residual untouched |
| `control_membership_admin.rs` | 12 | 1 | 11 converted (PR 4e); 1 permanent `ProdEnv::merge_peer` regression kept |

**Test-count trajectory** (`cargo test -p animusd --lib -- sim_cluster
--test-threads=2`, whole tier, each figure a landed PR's own attested
gate run except the PR 2+3 combined step, which neither PR's own commit
recorded — no `cargo` access at the time; first re-confirmed by PR 4a's
own baseline check): 478 (C-11 close) → 500 (PR 2+3, +22) → 522 (PR 4a,
+22) → 534 (PR 4b, +12) → 541 (PR 4c, +7) → 543 (PR 4d, +2) → **565
passed, 0 failed, 2 ignored (567 total)** (PR 4e, +22). Two full,
unfiltered `cargo test -p animusd --lib` runs (PR 4a, PR 4b) confirmed
zero regressions crate-wide: 700 passed/0 failed/3 ignored; 712
passed/0 failed/3 ignored. PR 4e's own gate re-confirmed the 567 total
via `cargo test -p animusd --lib -- --list sim_cluster` against four
module-filter-grouped runs covering every `sim_cluster_*` module at
least once, zero failures.

**The rung's own distinguishing lesson (this close-out's own correction,
2026-09-09): a rung's own opener plan and a residual's inherited "why it
stays `ProdEnv`" reason are both predictions/citations, not evidence.**
This close-out's first draft repeated both uncritically for `control_
membership_admin.rs` — "11 of 12 convert" never attempted by any landed
PR, and the 12th test's `--config`-bring-up label never re-read against
its own current body — until PR 4e (requested as a direct follow-up
after this close-out's own re-grep caught the gap) did the actual
conversion and, in the process, corrected the 12th test's label too. A
close-out must re-grep every named file against the landed PRs' actual
diffs and re-read each kept test's own current body before citing either
as fact; see `docs/engineering-lessons.md`'s C-12 close-out entry for the
generalized form (the same failure shape ADR 0061 rung K found for a
fixture-capability claim).
...
## Appendix — combined control-plane voter growth under SimCluster, C-14 closed (ADR 0061 rung N, PRs 1-5, 2026-09-14)

Folds the rung's own opener plus four PR-by-PR narratives (previously
inline in the "SimCluster: the multi-node generalization" section above)
into one coherent record now that the rung is closed — the fixture
surface, the two gotchas found along the way, and the final residue
table, not a chronological replay of every intermediate finding. See ADR
0061's "Rung N (post-C-13)" opener amendment through "Rung N closed" for
the full per-PR narrative, gate figures, and both real bugs found by
these PRs' own test authorship.

**Goal.** `SimCluster::grow`/`new_with_roles` both built every
control-bearing node ONCE, at construction time, as a mutual voter from
genesis — `self.controls: Vec<RaftNode<SimEnv>>` had no way to admit a
genuinely NEW `RaftNode<SimEnv>` to the LIVE quorum afterward
(`grow`'s own `assert_eq!(role, "data", ..)` panicked on anything else).
This rung gave `SimCluster` that primitive: a fresh, lone standalone
`RaftNode<SimEnv>` self-registered over the real relayed discovery path,
then admitted through the real `POST /admin/control/member/add` route —
never a control-plane bypass propose, mirroring production's own ADR
0037 admission sequence exactly.

**Primitives, as they stand after the rung.**

- **`control_index: BTreeMap<u64, usize>`** (node id → `self.controls`-
  vec index) and its inverse **`control_node_ids: Vec<u64>`**, with
  accessors **`control_index_of(node) -> Option<usize>`** /
  **`control_node_id(index) -> u64`** — the id/index generalization every
  growth and restart site now goes through, decoupling a grown voter's
  node id from its vec position so `self.controls` only ever grows by
  pushing at the end and no existing data node's id ever shifts. At
  construction, `control_node_ids[i] == i as u64` for every `i`; PR 2's
  own site-by-site re-audit confirmed the generalization touched exactly
  the sites PR 1's opener predicted (`restart`'s and `grow`'s
  `(0..self.controls.len())` voter-set derivations, `restart`'s `node <
  self.controls.len()` dispatch boundary, `new_with_roles`'s
  control-prefix assert's own doc) — no 2a/2b split needed.
- **`SimCluster::grow_control() -> u64`** — mints the next free node id
  (`self.nodes`, never reused); builds it a lone standalone `RaftNode<
  SimEnv>` via `RaftNode::start_with_metrics` whose own starting
  membership is `control_node_ids`' CURRENT value with `self` excluded —
  mirroring `join_control_nonvoter`'s real `bind_control`/
  `start_control_with` shape as closely as in-process construction
  allows; self-registers over the relayed `ProposeSchema`/`RegisterNode`
  path BEFORE ever trying to admit; admits via the real `POST /admin/
  control/member/add` route (`ClientCtx::admin_add_control_member`) —
  never a bypass, unlike `grow`'s own data-only self-registration;
  converges on every control-bearing node's own live voter belief
  (including the new node's own) actually including the new id before
  returning. `NodeRole::Control` only (no `DataRole`, no reconciler, no
  data-plane loop, no self-heartbeat) but DOES spawn the three
  control-plane-leader-gated janitors (`backup_janitor_loop`/
  `segment_janitor_loop`/`index_backfill_loop`), since this node can
  genuinely become control-plane leader. `restart`/`crash` are usable on
  the grown node's id immediately.
- **`SimCluster::grow_combined() -> u64`** (`grow("combined")`;
  `grow("data")` stays byte-identical to before this rung) — composes
  `grow_control`'s real voter admission with `grow`'s own pre-existing
  data-role assembly (full `ClientCtx`/`DataRole`/reconciler/heartbeat/
  TTL-reaper/auto-split, the fixture's own `RegisterNode{role:
  "combined"}`+`UpsertMember{Active}` bypass self-registration).
  Registration happens first, admission second, mirroring a real
  deployment's own order — the two are structurally independent axes in
  production (`Metadata::members`/`node_addrs` registration vs. a node's
  own local `RaftCore` becoming a live voter), so composing them needed
  no new design once traced. `SimCluster::restart` needed **zero**
  changes for this — C-12 (rung L, PR 2/3) already dispatches on
  `control_index_of` and gates each background loop on `role.
  has_data()`/`role.has_control()` generically; this rung is simply the
  first caller to exercise the `NodeRole::Both` half of that pre-existing
  support.

**Two gotchas found by these PRs' own test authorship, neither a defect
in the primitives themselves:**

1. **A crashed control node's own `is_leader()` belief stays frozen**
   (PR 2) — `SimCluster::crash` mutes a node, it does not stop it, so a
   crashed FORMER leader's own stale `true` belief sits at whatever it
   was the instant it crashed. A plain `control_leader_index()` scan
   (first-match-in-order) can keep returning that dead node's own vec
   index forever, silently misrouting every subsequent admin call — the
   third recorded occurrence of this crate's stale-self-belief class
   (`sim_cluster_auto_split.rs`'s own module doc for the CP-data-plane
   shape; C-12 PR 4b's `control_leader_index_excluding` for the control
   plane's own first occurrence). Fixed by routing every post-crash
   leader lookup through the pre-existing `control_leader_index_
   excluding`, keyed on the crashed node's own (stable) vec index — no
   new mechanism.
2. **A `propose()` right before a call that reads `Metadata` back needs
   virtual time in between** (PR 4) — calling `admin_add_control_member`
   immediately after two bypass proposes, with zero virtual time
   advanced, lets its own read-your-writes barrier capture a commit
   index at the barrier's own call start that predates either just-
   proposed entry actually committing; the barrier catches up short,
   `Metadata::node_addrs` reads as not-yet-containing the node, and the
   bounded retry-on-collision loop races the real (eventually-
   committing) registration to a permanent `409`. The fourth recorded
   instance of "advance virtual time before the next thing reads back
   what was just proposed" (after `sim_cluster_growth.rs`'s own scenario
   (d), `sim_cluster_backup_janitor.rs`'s propose-then-crash finding, and
   PR 2's own crashed-leader finding above) — fixed with a single `self.
   sim.run_for(Duration::from_millis(300))` margin between the proposes
   and the admin call. No production code changed either time —
   `admin_add_control_member`'s barrier behaved exactly as designed in
   both cases; the gap was each caller's own missing virtual-time margin.

**Test modules**: `sim_cluster_control_growth.rs` (4 tests — `grown_
control_voter_joins_the_live_quorum_and_can_lead`, `grow_control_after_
metadata_has_grown`, each pinned-seed plus a 5-seed `_over_seeds`
sibling; the shared `assert_grown_voter_crash_transfer_serve_and_restart`
proof: crash the pre-growth leader, a survivor wins a new election,
transfer leadership to the grown node over the real admin route, the
grown node commits a real member-remove for the crashed voter and every
survivor converges, the crashed node restarts and rejoins); `sim_cluster_
control_membership_split.rs` (4 tests — 2 existing from C-13 PR 6 + 2 new
from this rung's own PR 3, `grow_then_replace_a_voter_over_a_split_
deployment_with_live_data_traffic`'s sibling, built on `grow_control()`,
a fixed-victim-id strengthening exercising both the transfer-away and
no-transfer-needed branches across seeds); `sim_cluster_growth.rs`
scenario (f) (2 tests, `f_grow_combined_hosts_replicas_and_can_lead` —
`grow("combined")`, a rebalance-placed replica landing on the grown
node, then the identical crash/transfer/serve/restart proof duplicated
per this crate's own per-file-fixture-helper convention).

**Final residue table** — every real-socket control-voter-growth test
and its disposition:

| Consumer | Disposition |
|---|---|
| `control_membership_split.rs`'s one remaining test | **Converted** (PR 3) — real file deleted whole |
| `SimCluster::grow`'s deferred `"combined"` role arm | **Implemented** (PR 4) — `grow_combined()`, `grow("combined")` |
| Seed/join dial's `NodeRole::Control` option (3 `panic!` sites) | **Deferred, written verdict** (PR 4) — production's `--seed`/`join` dial has no `--role` flag and never produces a control voter at all; the three arms still `panic!`, messages rewritten to cite this finding |
| `heartbeat_live_destinations.rs`'s one test | **Permanent, real-socket** — `heartbeat_loop_live` is `ProdEnv`-hardcoded, `ProdEnv::merge_peer`'s peer-book scope limit is invisible under `SimEnv`, the identical structural reason `control_membership_admin.rs`'s own kept test stays real-socket — explicitly assessed and declined, not left unaddressed |

`ls crates/animusd/tests/ | grep -i 'control_membership\|heartbeat_live'`
confirms exactly two files remain: `control_membership_admin.rs` (its one
kept test, closed by C-12 PR 4e) and `heartbeat_live_destinations.rs`
(assessed by this rung) — both permanent, both with a documented reason.

**Zero production change** (confirmed via `git diff --stat origin/
main..HEAD -- crates/animusd/src/lib.rs crates/animusd/src/forwarding.rs`
across the whole rung): `forwarding.rs` untouched; `lib.rs` gained only
one additive `#[cfg(test)] mod sim_cluster_control_growth;` declaration
plus its own doc comment (PR 2's sole `lib.rs` edit). Every other new
method/field this rung added lives inside `sim_cluster.rs` behind
`#[cfg(test)]`; the only other production-adjacent touches were the
three rewritten panic messages and a doc-comment update on `grow` — no
dispatch behavior changed anywhere.

**Test-count trajectory** (`cargo test -p animusd --lib sim_cluster --
--test-threads=2`, whole tier): 581 (C-13 close) → 585 (PR 2, +4) → 587
(PR 3, +2) → **589 passed, 0 failed, 2 ignored** (PR 4, +2). PR 5 (this
close-out) adds 0 new tests. `Cargo.lock` unchanged throughout the rung.

**Next rung.** Both remaining real-socket files in this rung's own
subject area are already permanent (table above) — no further conversion
rung is identified for this consumer group. The separately-tracked "node
assembly/raw `ClientRequest`" group (2 files/8 tests, unowned since
C-08's own close-out) remains exactly that — unowned, un-opened, flagged
for an assess-and-close decision rather than a rung; this rung did not
touch it or change its status.
