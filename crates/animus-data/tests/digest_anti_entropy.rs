...
use animus_data::{DataClient, ReplicaHandle, TabletView, serve_anti_entropy, serve_replica};
use animus_env::EnvExt;
use animus_sim::{SimEnv, Simulator, TraceEvent};
...
fn digest_anti_entropy_converges_one_divergent_key_among_many() {
    ...
    let full_push_one_round: usize = handles
        ...
        .map(|h| {
            ...
            serde_json::to_vec(&animus_data::DataMsg::Sync {
                tablet: TABLET,
                epoch: Epoch::INITIAL,
                entries,
            })
...
fn converged_replicas_transfer_no_entry_data() {
    ...
    let one_digest = {
        let segs = animus_data::digest::digest(
            &futures::executor::block_on(handles[0].storage().entries_with_tombstones()).unwrap(),
        );
        serde_json::to_vec(&animus_data::DataMsg::SyncDigest {
            tablet: TABLET,
            epoch: Epoch::INITIAL,
            from: 0,
            segments: segs,
