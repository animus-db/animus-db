...
use animus_data::{DataClient, DataMsg, ReadResult, TabletView, serve_anti_entropy, serve_replica};
...
fn cluster(seed: u64, epoch: Epoch) -> (Simulator, Vec<animus_data::ReplicaHandle<MemoryEngine>>) {
...
fn value_at(handle: &animus_data::ReplicaHandle<MemoryEngine>, key: &[u8]) -> Option<Vec<u8>> {
...
fn raw_at(
    handle: &animus_data::ReplicaHandle<MemoryEngine>,
    key: &[u8],
...
fn start_anti_entropy(sim: &Simulator, handles: &[animus_data::ReplicaHandle<MemoryEngine>]) {
    for &id in &REPLICAS {
        serve_anti_entropy(
            sim.env(id),
            handles[id as usize].clone(),
